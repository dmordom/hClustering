...
void WHtreePartition::scanOptimalPartitions( const size_t levelDepth, std::vector< float > &partitionValues,
                                             std::vector< std::vector< size_t> > &partitionVector, const bool verbose )
{
    ...
    {
        ++stepNr;
        std::cout << "\rStep: " << stepNr << ". Current partition size: " << currentPartition.size() << ". Current value: "<< currentValue << "       "<< std::flush;
        ...
        size_t bestValueIndex( 0 ); // keeps the first branching index of the best partition
    ...
    std::cout<<"250 % match: " <<(match250*100.0)/(match250+mismatch250) << ". Total matches: "<< match250 << ". fails: "<< mismatch250 <<std::endl;
    std::cout<<"500 % match: " <<(match500*100.0)/(match500+mismatch500) << ". Total matches: "<< match500 << ". fails: "<< mismatch500 <<std::endl;
    std::cout<<"1000 % match: " <<(match1000*100.0)/(match1000+mismatch1000) << ". Total matches: "<< match1000 << ". fails: "<< mismatch1000 <<std::endl;
    std::cout<<"2000 % match: " <<(match2000*100.0)/(match2000+mismatch2000) << ". Total matches: "<< match2000 << ". fails: "<< mismatch2000 <<std::endl;
    std::cout<<"5000 % match: " <<(match5000*100.0)/(match5000+mismatch5000) << ". Total matches: "<< match5000 << ". fails: "<< mismatch5000 <<std::endl;
...
void WHtreePartition::scanHozPartitions( std::vector< float > &partitionValues,
                                             std::vector< std::vector< size_t> > &partitionVector, const bool verbose )
{
    partitionValues.clear();
    partitionVector.clear();
...
size_t WHtreePartition::filterMaxPartitions( const unsigned int filterRadius, std::vector< float > *partValPoint, std::vector< std::vector< size_t> > *partVectorPoint )
{
...
void WHtreePartition::writePartitionSet( std::string partFileName, const std::vector< float > &partitionValues, const std::vector< std::vector< size_t> > &patitionVector )
{
...
float WHtreePartition::evalSSindex( const std::vector<nodeID_t> &partition ) const
{
                                ...
                                const bool excludeLeaves, const size_t root, const size_t levelDepth ) const
 {
     ...
     while( loopCondition )
     {
         ...
         std::vector< std::vector< unsigned int > > derivedIndexes( m_tree.getBranching(bestPartition, levelDepth, &derivedPartitionSet, excludeLeaves));
 ...
 float WHtreePartition::partitionSharp( const float compValue, std::vector<nodeID_t>* partition, const bool excludeLeaves, const size_t root, const bool normalized ) const
 {
     partition->clear();
...
std::vector< std::vector< dist_t > > WHtreePartition::getICDmatrix( const std::vector<size_t> &oldPartition, size_t branchPos, std::vector<size_t> branch,
                                                                    std::vector< std::vector< dist_t > > oldMatrix )
{
 ...
 void WHtreePartition::level2granularity( std::vector<nodeID_t>* const partitionPointer ) const
 {
 ...
 void WHtreePartition::level2granularity( std::vector<size_t>* const partitionPointer ) const
 {
