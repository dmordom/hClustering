...
/**
 * Class implements a hierarchical tree and provides functions for creation, partitioning, selection and navigation
 */
class WHtree
{
public:
    ...
    WHtree( std::string treeName, WHcoord datasetSizeInit, std::vector<WHnode> leavesInit, std::vector<WHnode> nodesInit,
    ...
    //! Returns the values for the selected partitions
    //! \return list containing the values for the selected partitions
    ...
    //! \param nodeID1 first coordinate to compute distance from
    //! \param nodeID2 second ncoordinate to compute distance from
    //! \return cophenetic distance
    ...
    void sortBySize( std::list<size_t>* const nodeVector ) const;


    //! Reorders the nodes in the vector according to their size
    //! \param nodeVector vector of nodes to be reordered
    void sortBySize( std::vector<nodeID_t>* const nodeVector ) const;
    void sortBySize( std::list<nodeID_t>* const nodeVector ) const;
    ...
    void sortByHLevel( std::list<size_t>* const nodeVector ) const;


    //! Reorders the nodes in the vector according to their hierarchical level
    //! \param nodeVector vector of nodes to be reordered
    void sortByHLevel( std::vector<nodeID_t>* const nodeVector ) const;
    void sortByHLevel( std::list<nodeID_t>* const nodeVector ) const;
    ...
    //! \return success indicator
    bool writeTree( const std::string &filename ) const;
...
private:
    ...
    //! implements a compare operator for nodes, depending on the Size
    struct compSize
    {
