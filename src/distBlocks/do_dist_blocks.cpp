...
void do_dist_blocks(std::string tract_dir,
    ...
    if (rand_mode) {
        ...
        rand_tracts.resize(roivect.size());
...
//            std::cout <<"Generating tractograms uniformly distributed over the hypersphere surface, Tract size: "<<rand_tract_lenght<<"..."<< std::endl;
//            rand_tracts.reserve(roivect.size());
//            while (rand_tracts.size()<roivect.size()) {
...
//                }
//            progress = (rand_tracts.size()*100.)/(roivect.size());
        ...
        //--
            ...
            for (int i=0; i<roivect.size(); ++i) {
                ...
                write_Vtract(get_Vname(out_dir+"/tracts",roivect[i]),rand_tracts[i]);
    ...
    size_t num_dist_blocks(roivect.size()/bsize);
    ...
    for (int row=0 ; row < num_dist_blocks ; ++row) {
        ...
        if (bsize==tract_block_size && !rand_mode) {
            ...
            for (int col=row ; col < num_dist_blocks ; ++col) {
                ...
                if (roivect.size()%bsize) { // not all blocks wil be square
                    if ( (row<num_dist_blocks-1)&&(col<num_dist_blocks-1) ) { //its one of the full square blocks
                        ...
                        std::vector<float> test_vect(roivect.size()%bsize,0);
                        std::vector<std::vector<float> > test_block(roivect.size()%bsize,test_vect);
                        dist_block.swap(test_block);
                ...
                if (row==col) { // block is on the diagonal
                    ...
                    if (postlast_col_seed>roivect.size())
                        postlast_col_seed = roivect.size();

                    std::vector<coordinate> seed_set_col(&roivect[first_col_seed], &roivect[postlast_col_seed]);
            ...
            for (int col=row ; col < num_dist_blocks ; ++col) {
                ...
                if (row==col) { // block is on the diagonal
                    ...
                    if (rand_mode) {
                        ...
                        fill_dist_block(tract_dir, seed_set_row, empty_coord, dist_block, tract_block_size, tract_length, threads, verbose, veryvb);
                    }
