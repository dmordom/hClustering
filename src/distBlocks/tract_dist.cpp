...
std::vector<float> join_tracts(std::vector<float> tractogram1, std::vector<float> tractogram2, nodes_size_t nleaves1, nodes_size_t nleaves2) {
    if (tractogram1.size()!=tractogram2.size())
        throw std::runtime_error ("ERROR [join_tracts()]: Tractograms are not of the same size");
    unLog(tractogram1);
    unLog(tractogram2);
    std::vector<float> new_tractogram;
    std::vector<float>::const_iterator iter1;
    std::vector<float>::const_iterator iter2;

    new_tractogram.reserve(tractogram1.size()); // reserve memory to increase speed

    for(iter1=tractogram1.begin(),iter2=tractogram2.begin(); iter1 != tractogram1.end() ; ++iter1, ++iter2 ) {
        new_tractogram.push_back( ((*iter1 * nleaves1)+(*iter2 * nleaves2)) /(nleaves1+nleaves2) );
    }
    doLog(new_tractogram);
...
void unLog(std::vector<float> &tractogram) {
    for(std::vector<float>::iterator iter=tractogram.begin() ; iter!=tractogram.end() ; ++iter)
        *iter = pow (10.,*iter);
...
//void threshold(std::vector<float> &tractogram, float threshold) {
//    if (threshold!=0) {
