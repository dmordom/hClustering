...
//
//   -T --tempf:	  Mean tractogram folder location for centroid method. Will be used as temporary storage for intermediate tractogram results.
//
...
//
//  [--nolog]:        Use if input tracts are only linearly normalized instead of logarithmically normalized (assumed by default).
//
...
//  * Usage example:
//
//   buildctree -r roi_lh.txt -I tracograms/ -T /tmp/tracts -O results/ -t 0.001 -d 0.1 -c 26 -N 1000 -k -m 2 -v
//
//
//  * Outputs (in output folder defined at option -O):
//
//   - 'cX.txt' - (where X is the neighborhood level defined at option -c) Contains the output hierarchical tree.
