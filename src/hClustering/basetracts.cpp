...
//    try {
        ...
        std::string progName("basetracts");
        ...
        bool useFloat( false ), doZip(true), onlymasks(false);
        ...
        genericOptions.add_options()
                ...
                ("ufloat,u", "write tractograms in float instead of char")
                ("nozip,z", "write tractograms unzipped")
                ("onlymasks", "write only cluster masks")
                ("tract-folder,f", boost::program_options::value< std::string >(&tractFolder), "folder with the single-voxel probabilistic tracts")
                ("mask,m", boost::program_options::value< std::string >(&maskFilename), "file with the white matter mask")
        ...
        if (variableMap.count("ufloat")) {
            std::cout << "Saving node tractograms in float representation"<<std::endl;
            useFloat=true;
        }
        else
        {
            std::cout << "Saving node tractograms in char representation"<<std::endl;
            useFloat=false;
        }

        if (variableMap.count("nozip")) {
            std::cout << "Saving node tractograms non zipped"<<std::endl;
            doZip=false;
        }
        else
        {
            std::cout << "Saving node tractograms zipped"<<std::endl;
            doZip=true;
        }

        if (variableMap.count("onlymasks")) {
            std::cout << "writing only cluster masks"<<std::endl;
            onlymasks=true;
        }
        ...
        if (variableMap.count("mask")) {
            ...
            std::cout << "WM mask file: "<< maskFilename << std::endl;
        } else {
        ...
        logFile << "WM mask file: "<< maskFilename << std::endl;
        logFile <<"Tract folder:\t"<< tractFolder <<std::endl;
        logFile <<"Output folder:\t"<< outputFolder <<std::endl;
        logFile <<"Use float :\t"<< useFloat <<std::endl;
        logFile <<"DoZip :\t"<< doZip <<std::endl;
        ...
        {
            std::cerr<< "WARNING: tree is not base node tree"<<std::endl;
        }
        ...
        treeMngr.writeFullBaseNodeTracts(useFloat,doZip,onlymasks);
