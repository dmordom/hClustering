...
//    try {
        ...
        std::string progName("full2compacttract");
        ...
        if (variableMap.count("output")) {
            ...
            std::cerr << "ERROR: no output filename stated"<<std::endl;
            std::cerr << visibleOptions << std::endl;
            exit(-1);
        }
        ...
        else
        {
            ...
            vistaMngr.readFullTract( tractFilename, &simpleTract );
            vistaMngr.writeTract( outputFilename,simpleTract );
