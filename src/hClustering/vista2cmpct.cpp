...
//
//  WARNING: this program will port the vector as-is, and is meant simply for completion of the cmpct2vista program.
...
//
//   -i --input:      [mutually exclusive with -f] Input vista 1D vector to be converted into .cmpct format, multiple inputs allowed separated by spaces.
//
...
//    try {
        ...
        std::string progName("vista2cmpct");
        ...
        if (variableMap.count("help"))
        {
            ...
            std::cout << "vista2cmpct" << std::endl << std::endl;
            std::cout << "Transform a 1D .v vista image vector into a 1D .cmpct compact tract vector." << std::endl << std::endl;
            ...
            std::cout << "[-o --output]:    Output file or folder to write .cmpct tracts." << std::endl << std::endl;
        ...
        for( size_t index = 0; index < finalInputs.size(); ++index )
        {
            ...
            if(!outputFilename.empty())
            {
                ...
                if(outIsFolder)
                {
                    outPath = outputFilename + "/" + stem + SUFFIX + COMPACT_EXT;
                }
                else
                {
