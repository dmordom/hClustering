...
void do_dist_blocks(std::string tract_dir,
    ...
    // variable for random mode
    const size_t rand_tract_lenght = 10;
    std::vector< std::vector<float> > rand_tracts;
    ...
    if (rand_mode) {
        ...
        time_t rand_start(time(NULL)), rand_current_time(time(NULL));
        size_t progress_int(0), rand_prog_time(0), expected_remain(0);
        float progress(0);
        ...
        boost::variate_generator<boost::mt19937, boost::normal_distribution<> >  randn_prng(igen, boost::normal_distribution<>(0,1));

        std::cout <<"Generating tractograms uniformly distributed over the hypersphere surface, Tract size: "<<rand_tract_lenght<<"..."<< std::endl;
        rand_tracts.resize(roiNum);
        for (int i=0; i<roiNum; ++i) {

            std::vector<float> rand_tract;
            rand_tract.reserve(rand_tract_lenght);
            float radius(0);
            // first create a vector of normally distributed values
            for (int j=0; j<rand_tract_lenght; ++j ){
                float value(fabs(randn_prng()));
                rand_tract.push_back((value));
                radius+=value*value;
            }
            ...
            for (int j=0; j<rand_tract.size(); ++j ){
                rand_tract[j]=(rand_tract[j]/radius);
            }
            ...
            rand_tracts[i]=rand_tract;
            progress = (i*100.)/(roiNum);
...
//            boost::variate_generator<boost::mt19937, boost::uniform_01<> >  u01_prng(igen, boost::uniform_01<>());

//            std::cout <<"Generating tractograms uniformly distributed over the hypersphere surface, Tract size: "<<rand_tract_lenght<<"..."<< std::endl;
//            rand_tracts.reserve(roiNum);
//            while (rand_tracts.size()<roiNum) {
...
//                if (norm<=1) {
//                    for (int j=0; j<rand_tract.size(); ++j ){
//                        rand_tract[j]=(rand_tract[j]/norm);
//                    }
//                    rand_tracts.push_back(rand_tract);
//                }
//            progress = (rand_tracts.size()*100.)/(roiNum);
        ...
        //--
                ...
                rand_current_time = (time(NULL));
                rand_prog_time = ( difftime(rand_current_time,rand_start) );
                expected_remain = (rand_prog_time*((100.-progress)/progress));
                std::cerr <<"\r"<< std::flush << progress_int <<" % of tractograms generated. Expected remaining time: ";
                std::cerr<<expected_remain/3600 <<"h "<<  (expected_remain%3600)/60 <<"' "<< ((expected_remain%3600)%60) <<"\"   "<< std::flush;
            ...
            std::cout <<"Writing tractograms to file..."<< std::endl;
            rand_start=(time(NULL));
            for (int i=0; i<roiNum; ++i) {

                write_Vtract(get_Vname(out_dir+"/tracts",i),rand_tracts[i]);
                ...
                rand_prog_time = ( difftime(rand_current_time,rand_start) );
                expected_remain = (rand_prog_time*((100.-progress)/progress));
                std::cerr <<"\r"<< std::flush << progress_int <<" % of tractograms written. Expected remaining time: ";
                std::cerr<<expected_remain/3600 <<"h "<<  (expected_remain%3600)/60 <<"' "<< ((expected_remain%3600)%60) <<"\"   "<< std::flush;
            }
            std::cout << std::endl;
    ...
    // calculate the number of blocks to be written from the size of the seed set
    size_t num_dist_blocks(roiNum/bsize);
    if (roiNum%bsize)
        ++num_dist_blocks;
    ...
    for (int row=0 ; row < num_dist_blocks ; ++row) {
        ...
        {
            seed_set_row.push_back(i);
        }

        std::vector< std::vector<float> > rand_tracts_row;
        if (rand_mode)
            rand_tracts_row.assign(&rand_tracts[first_row_seed],&rand_tracts[postlast_row_seed]);



        if (bsize==tract_block_size && !rand_mode) {
            ...
            unsigned char *row_tract_block = new unsigned char [tract_length*seed_set_row.size()];
            double *row_sqrsum = new double[seed_set_row.size()];

            //load data into block
            load_tract_block(tract_dir, seed_set_row, seed_set_row.size(), tract_length, row_tract_block);

            //compute square sum
            do_sqrsum(row_sqrsum, row_tract_block, seed_set_row.size(), tract_length, threads);


            // loop through columns of the distance matrix (each element will be a separate distance block)
            for (int col=row ; col < num_dist_blocks ; ++col) {
                ...
                if (roiNum%bsize) { // not all blocks wil be square
                    if ( (row<num_dist_blocks-1)&&(col<num_dist_blocks-1) ) { //its one of the full square blocks
                        std::vector<float> test_vect(bsize,0);
                        std::vector<std::vector<float> > test_block(bsize,test_vect);
                        dist_block.swap(test_block);
                    } else if ( (row==num_dist_blocks-1)&&(col==num_dist_blocks-1) ) { // its the square block in the lower right corner
                        std::vector<float> test_vect(roiNum%bsize,0);
                        std::vector<std::vector<float> > test_block(roiNum%bsize,test_vect);
                        dist_block.swap(test_block);
                    } else { // its one of the rectangular blocks in the final column
                        std::vector<float> test_vect(roiNum%bsize,0);
                        std::vector<std::vector<float> > test_block(bsize,test_vect);
                        dist_block.swap(test_block);
                    }
                } else { // all blocks will be square
                    std::vector<float> test_vect(bsize,0);
                    std::vector<std::vector<float> > test_block(bsize,test_vect);
                    dist_block.swap(test_block);
                }
                ...
                if (row==col) { // block is on the diagonal

                    std::cout <<"transposing..."<< std::flush;


                    // get transposed version of the block
                    unsigned char *row_tract_block_tp = new unsigned char [tract_length*seed_set_row.size()];

                    for(size_t i=0; i<seed_set_row.size();++i) {
                        for(size_t j=0; j<tract_length;++j) {
                            row_tract_block_tp[(j*seed_set_row.size())+i]=row_tract_block[(i*tract_length)+j];
                        }
                    }


                    // Correlate row block with itself
                    compute_dist_block(dist_block, row_tract_block, row_tract_block_tp, row_sqrsum, row_sqrsum, 0, 0, seed_set_row.size(), seed_set_row.size(), tract_length, threads);
                    ...
                    size_t first_col_seed(col*bsize), postlast_col_seed((col+1)*bsize);
                    if (postlast_col_seed>roiNum)
                        postlast_col_seed = roiNum;

                    std::vector<size_t> seed_set_col;
                    for( size_t i=first_col_seed; i < postlast_col_seed; ++i)
                    {
                        seed_set_col.push_back(i);
                    }



                    //allocate memory for tractogram block, square sum and create cropped seed vector
                    unsigned char *col_tract_block = new unsigned char [tract_length*seed_set_col.size()];
                    ...
                    load_tract_block_transposed(tract_dir, seed_set_col, seed_set_col.size(), tract_length, col_tract_block);


                    //compute square sum
                    do_sqrsum_transposed(col_sqrsum, col_tract_block, seed_set_col.size(), tract_length, threads);

                    // Correlate row block with with col block
                    compute_dist_block(dist_block, row_tract_block, col_tract_block, row_sqrsum, col_sqrsum, 0, 0, seed_set_row.size(), seed_set_col.size(), tract_length, threads);
                ...
                // printout progress
                progress = (block_prog*100.)/(num_dist_blocks*(num_dist_blocks+1)/2);
                progress_int = progress;
                ...
                if (!((row==num_dist_blocks-1)&&(col==num_dist_blocks-1)))
            ...
            for (int col=row ; col < num_dist_blocks ; ++col) {
                ...
                if (row==col) { // block is on the diagonal
                    ...
                    std::vector<size_t> empty_seed;
                    std::vector<std::vector<float> > empty_tracts;


                    //fill distance block matrix
                    if (rand_mode) {
                        fill_rand_block(rand_tracts_row, empty_tracts, dist_block, rand_tract_lenght, threads, verbose, veryvb);
                    } else {
                        fill_dist_block(tract_dir, seed_set_row, empty_seed, dist_block, tract_block_size, tract_length, threads, verbose, veryvb);
                    }
                ...
                } else { // block is over the diagonal
                    ...
                    //fill distance block matrix
                    if (rand_mode) {
                        fill_rand_block(rand_tracts_row, rand_tracts_col, dist_block, rand_tract_lenght, threads, verbose, veryvb);
                    } else {
                        fill_dist_block(tract_dir, seed_set_row, seed_set_col, dist_block, tract_block_size,tract_length, threads, verbose, veryvb);
                    }
                ...
                // printout progress
                progress = (block_prog*100.)/(num_dist_blocks*(num_dist_blocks));
                progress_int = progress;
...
void fill_dist_block(std::string &tract_dir,
                     std::vector<size_t> &row_dist_seed_set,
                     std::vector<size_t> &col_dist_seed_set,
                     std::vector<std::vector<float> > &dist_block,
                     const size_t &tract_block_size,
                     const size_t &tract_length,
                     const size_t &threads,
    ...
    for (int row=0; row<num_row_tblocks ; ++row) {
        ...
        std::vector<size_t> row_tblock_seed_set(&row_dist_seed_set[row_distb_offset],&row_dist_seed_set[row_distb_offset+row_tblock_size]);
        ...
        if (col_dist_seed_set.empty()) {   // distance block is part of the diagonal, only compute upper diagonal values
            ...
            for (int col=row; col<num_row_tblocks ; ++col) {
                ...
                if (col==row) { // sub-block is also on the diagonal
                    ...
                    std::vector<size_t> col_tblock_seed_set(&row_dist_seed_set[col_distb_offset],&row_dist_seed_set[col_distb_offset+col_tblock_size]);
            ...
            for (int col=0; col<num_col_tblocks ; ++col) {
                ...
                unsigned char *col_tract_block = new unsigned char [tract_length*col_tblock_size];
                double *col_sqrsum = new double[col_tblock_size];
                std::vector<size_t> col_tblock_seed_set(&col_dist_seed_set[col_distb_offset],&col_dist_seed_set[col_distb_offset+col_tblock_size]);
    ...
    if (veryvb) {
        ...
        for (std::vector<std::vector<float> >::const_iterator iter_row(dist_block.begin()); iter_row!=dist_block.end(); ++iter_row) {

            for (std::vector<float>::const_iterator iter_col(iter_row->begin()); iter_col!=iter_row->end(); ++iter_col) {
...
void load_tract_block(std::string tract_dir,
    ...
    for(int i=0; i < tract_block_size; ++i) {
        ...
        size_t current_seed(seed_set[i]);
        get_Vtract_th(get_Vname(tract_dir,current_seed),&tract_block[i*tract_length],tract_length, threshold);
...
void load_tract_block_transposed(std::string tract_dir,
    ...
    for(int i=0; i < tract_block_size; ++i) {
        ...
        size_t current_seed(seed_set[i]);
        get_Vtract_th(get_Vname(tract_dir,current_seed),current_tract,tract_length, threshold);
